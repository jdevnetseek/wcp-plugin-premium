...
function wcp_get_more_info_template( $wcp_data_ID, $ndf_data_results_layout, $last_data_row_class, $grid_moreinfo_column_class, $more_fields_more_info, $field_info ){
	...
	if( !empty( $grid_moreinfo_column_class ) ){
		...
		if($ndf_data_enable_request_form_meta_box['ndf_data_enable_request_form_meta_box'][0] == 1){
				...
				$output .= '<button class="request-quotes-single get-quotes-buttons" btn-title="'.$request_quotes_form_submit_button_text.'"  data-title="'.$request_quotes_form_title.'" subtitle="'.$request_quotes_form_subtitle.'" id="request-quotes-single-horizontal" data-modal="'.$wcp_data_ID.'" >'.$single_email_request_quotes_form_title_button.'</button>';

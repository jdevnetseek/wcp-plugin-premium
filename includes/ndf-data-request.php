...
// FUNCTION TO SEND QUOTES TO TOP RATED WCP DATA
function callback_send_toprated_email(){
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
		global $wpdb;
        $listofperson = $wpdb->get_results("SELECT * FROM $wpdb->postmeta WHERE meta_key = 'ndf_data_recipient_email'" ,ARRAY_A);
		// $toprated = $wpdb->get_results("SELECT * FROM $wpdb->postmeta WHERE meta_key = 'ndf_data_star_rating' AND meta_value = '5'"  , ARRAY_A);
		$toprated = $wpdb->get_results("SELECT * FROM $wpdb->postmeta WHERE meta_key = 'ndf_data_star_rating'", ARRAY_A);
		
		if (!empty($toprated)) {
			$star = [];
			foreach($toprated as $wcpdata){
				if($wcpdata['meta_value'] != 'custom-rating' &&  $wcpdata['meta_value'] != 'no-rating'){
					$star [] = $wcpdata['meta_value'];
				}
			}
			$toprated_data = max($star);
			$custom_rating = $wpdb->get_results("SELECT * FROM $wpdb->postmeta WHERE meta_key = 'mrp_rating_result_1_star_rating' AND meta_value = '$toprated_data'", ARRAY_A);

			$combined_data = array();
			$count_top_rated = array();
			foreach($toprated as $wcpdata){
				if($wcpdata['meta_value'] == $toprated_data){ 
					$count_top_rated []= $wcpdata;
				}
			}
			// print_r($count_top_rated);
			$combined_data = array_merge($custom_rating,$count_top_rated);
			// print_r($combined_data);
			$toprated_array = array();
			$listofperson_array = array();
			foreach($combined_data as $toprated_value){
				$toprated_array [] = $toprated_value['post_id'];			
			}
			// print_r($toprated_array);
			foreach ($listofperson as $listofperson_value) {
				array_push($listofperson_array,array("post_id" => $listofperson_value['post_id'], "email" => $listofperson_value['meta_value']));			
			}
			$recepients_to_send = array();
			for ($i=0; $i < count($toprated_array); $i++) { 
				for ($j=0; $j < count($listofperson_array); $j++) { 
					if ($toprated_array[$i] === $listofperson_array[$j]['post_id']) {
						$recepients_to_send [] = $listofperson_array[$j]['email'];
					}
				}
			}
			// var_dump($recepients_to_send);
			// var_dump($toprated);
			...
			$headers = array('Content-Type: text/html; charset=UTF-8');
			  ...
			  foreach($recepients_to_send as $filter_valid_recipients):
			  ...
			  if(count($count_valid_recipients ) < 3)
			  {
				foreach ($count_valid_recipients as $valid_recipients) {
					...
					while($counter < 3):
						$random_numbers = array_rand($recepients_to_send);
							if(!empty($recepients_to_send[$random_numbers]))
							{
							if(in_array($recepients_to_send[$random_numbers],$to_send)){ /* do something */}
								else {    $to_send [] = $recepients_to_send[$random_numbers]; $counter++; }
							}
						else{ /*  do something */ }
					endwhile;
...
add_action( 'wp_ajax_send_toprated_email', 'callback_send_toprated_email' );
add_action( 'wp_ajax_nopriv_send_toprated_email', 'callback_send_email' );
// END OF FUNCTION TO SEND QUOTES TO TOP RATED WCP DATA

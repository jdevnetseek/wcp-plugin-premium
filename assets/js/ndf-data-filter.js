...
$(function($) {
	...
	$("#wcp_keyword_search").on('keyup',function(event){
		if(event.keyCode == 13){
	...
	$('#filters_slider').on('afterChange', function(event, slick, currentSlide){
		...
		// for request-form-quotes single
	...
	$(document).on('click', '#request-quotes-btn', function(e) {
		...
		let checkbox = $('#checkbox-id-popup').val();
			if(name != ''){
				var validated_name = true;
			}
			else{
		...
		if(validated_name == true && validated_email == true && validated_message == true && checkbox==false){
			jQuery.ajax({
				url: url,
				type: "POST",
				cache: false,
				...
				complete: function(){
					...
					$('#checkbox-id-popup').prop('checked', false);
					  }
			});
		}
		else if(validated_name == true && validated_email == true && validated_message == true && checkbox==true){
			jQuery.ajax({
				url:url,
				type:'POST',
				cache:false,
				data:{
					action: 'send_toprated_email', 
					name: name,
					email: email,
					phone:phone,
					request:request
